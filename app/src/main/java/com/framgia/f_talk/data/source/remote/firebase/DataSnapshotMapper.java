...
import com.framgia.f_talk.data.source.remote.firebase.exception.FirebaseDataCastException;
...
public abstract class DataSnapshotMapper<T, U> implements Function<T, U> {
    ...
    public static <U> DataSnapshotMapper<DataSnapshot, List<U>>
    listOf(Class<U> clazz, Function<DataSnapshot, U> mapper) {
        return new TypedListDataSnapshotMapper<>(clazz, mapper);
    }

    public static <U> DataSnapshotMapper<DataSnapshot, LinkedHashMap<String, U>>
    mapOf(Class<U> clazz) {
        return new TypedMapDataSnapshotMapper<>(clazz);
    }

    public static <U> DataSnapshotMapper<DataSnapshot, U> of(GenericTypeIndicator<U>
                                                                     genericTypeIndicator) {
        return new GenericTypedDataSnapshotMapper<U>(genericTypeIndicator);
    }

    public static <U> DataSnapshotMapper<FirebaseChildEvent<DataSnapshot>, FirebaseChildEvent<U>>
    ofChildEvent(Class<U> clazz) {
    ...
    private static <U> U getDataSnapshotTypedValue(DataSnapshot dataSnapshot, Class<U> clazz) {
        ...
        try {
            ...
            throw Exceptions.propagate(new FirebaseDataCastException(
                    StringUtil.append(dataSnapshot.toString(), clazz.getSimpleName()), ex));
        }
        if (value == null) {
            throw Exceptions.propagate(new FirebaseDataCastException(
                    StringUtil.append(dataSnapshot.toString(), clazz.getSimpleName())));
        }
        return value;
    }
    ...
    private static class TypedListDataSnapshotMapper<U> extends DataSnapshotMapper<DataSnapshot,
            List<U>> {
    ...
    private static class TypedMapDataSnapshotMapper<U> extends
    ...
    private static class GenericTypedDataSnapshotMapper<U> extends
    ...
    private static class ChildEventDataSnapshotMapper<U>
        ...
        @Override
        public FirebaseChildEvent<U> apply(final FirebaseChildEvent<DataSnapshot>
                                                           firebaseChildEvent) {
            DataSnapshot dataSnapshot = firebaseChildEvent.getValue();
            if (dataSnapshot.exists()) {
                return new FirebaseChildEvent<U>(
                        dataSnapshot.getKey(),
                        getDataSnapshotTypedValue(dataSnapshot, clazz),
                        firebaseChildEvent.getPreviousChildName(),
                        firebaseChildEvent.getEventType());
            } else {
    ...
    public static final Predicate<DataSnapshot> DATA_SNAPSHOT_EXISTENCE_PREDICATE =
